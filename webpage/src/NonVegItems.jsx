...
const NonVegItems = () => {
  ...
  useEffect(() => {
    api.get("/menu").then((res) => setNonVegItems(res.data.filter((i) => i.category === "Non-Veg")));
  }, []);

  return (
    <div className="p-8 bg-amber-50 min-h-screen">
      <h2 className="text-3xl font-bold mb-6 text-amber-900">🍗 Non-Veg Items</h2>
      <div className="grid md:grid-cols-3 gap-6">
        ...
        {nonVegItems.length === 0 && <p className="text-gray-500">No non-veg items found.</p>}
      </div>
    </div>
  );
};
